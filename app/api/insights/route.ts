...
import { CalendarInsightsService } from '@/services/calendarInsights';
...
export async function GET(req: NextRequest) {
  try {
    ...
    // Generate insights
    const insights = CalendarInsightsService.generateInsights(events, metadata);
    ...
    return NextResponse.json(
      { error: 'Failed to fetch calendar insights', details: error.message },
      { status: 500 }
    );
  }
}
